...
| `WithScopeTemplate(tmpl)` | An option to `Generate`: the scope is rendered for that one call through `tmpl`, a `*text/template.Template` whose argument is a `gimble.ScopeData` (`Values` outermost first, `By` keyed; each has `Key`, `Value`, `Text`). | The prompt stays a constant and the template is parsed once at package level, from a constant or a `go:embed` file, so both stay readable in the source. A template that cannot render is the error `Generate` returns. |
