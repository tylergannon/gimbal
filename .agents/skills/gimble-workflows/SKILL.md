...
| `PromiseLoop(ctx, name, goal, planner, opts...)` | The planner keeps a backlog and picks each next `Task`; range over `loop.Tasks`, then check `loop.Err()`. | Each task is a scope. What the body `Set`s is what the planner sees next. It ends by picking no task. `WithSupervisor` watches every planning turn while it runs, but its interval does not guarantee a look before a short turn ends. |
