...
func TestRunsCommandIgnoresStaleDiscovery(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(controlDir, "stale.json"), []byte(`{"pid":1234,"socket":"/tmp/gimbal-missing.sock"}`), 0o644); err != nil {
		t.Fatal(err)
	}
	var output bytes.Buffer
...
func TestWatchRunReadsSnapshotAndSSEFromRuntime(t *testing.T) {
	...
	go func() {
		runDone <- runtime.Run(ctx, "watched", map[gimbal.WorkflowRole]gimbal.ModelBinding{
			"coder": {Adapter: adapter, Model: "m"},
		}, func(ctx context.Context) error {
			return gimbal.Scope(ctx, "lap", func(ctx context.Context) error {
				session := gimbal.NewSession(ctx, "coder", "/w")
				_, err := session.Generate[gimbal.Text](ctx, "wait")
				return err
			})
		})
	}()
...
func (b *watchBlocking) RunTurn(ctx context.Context, _, _ string, _ json.RawMessage, _ func(gimbal.AgentEvent) error) (gimbal.TurnResult, error) {
