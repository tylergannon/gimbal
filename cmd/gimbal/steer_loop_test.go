...
func TestSteerCommandQueuesForNextPlanningDecision(t *testing.T) {
	...
	go func() {
		done <- runtime.Run(ctx, "queued", map[gimbal.WorkflowRole]gimbal.ModelBinding{
			"planner": {Adapter: planner, Model: "m"},
		}, func(ctx context.Context) error {
			session := gimbal.NewSession(ctx, "planner", project)
			loop := gimbal.PromiseLoop(ctx, "plan", "Finish the task", session)
	...
	runID := oneCLIRunID(t, filepath.Join(project, ".gimbal"))
...
func (p *queuedPlanner) RunTurn(_ context.Context, _, prompt string, _ json.RawMessage, _ func(gimbal.AgentEvent) error) (gimbal.TurnResult, error) {
	p.prompts <- prompt
	p.calls++
	if p.calls == 1 {
		return gimbal.TurnResult{Output: json.RawMessage(`{"tasks":[{"name":"one","description":"work","definition_of_done":"finish","validation":{"command":"","query":""}}],"next":0}`)}, nil
	}
