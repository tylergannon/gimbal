...
func TestSteerCommandDeliversToRunningSession(t *testing.T) {
	...
	runWG.Go(func() {
		_ = runtime.Run(ctx, "steering", map[gimbal.WorkflowRole]gimbal.ModelBinding{
			...
			return gimbal.Scope(ctx, "lap", func(ctx context.Context) error {
				...
				_, err := session.Generate[gimbal.Text](ctx, "active")
				return err
			})
		})
	})
...
func TestSteerCommandReportsIdleSessionAsDropped(t *testing.T) {
	...
	runWG.Go(func() {
		_ = runtime.Run(ctx, "steering", map[gimbal.WorkflowRole]gimbal.ModelBinding{
			...
			return gimbal.Scope(ctx, "lap", func(ctx context.Context) error {
				session := gimbal.NewSession(ctx, "coder", "/w")
				_, err := session.Generate[gimbal.Text](ctx, "idle")
				close(adapter.idle)
				<-adapter.release
				return err
			})
...
func (a *steerAdapter) RunTurn(ctx context.Context, _, prompt string, _ json.RawMessage, _ func(gimbal.AgentEvent) error) (gimbal.TurnResult, error) {
	a.mu.Lock()
	a.active = true
	a.mu.Unlock()
	select {
	...
	if prompt == "idle" {
		return gimbal.TurnResult{Output: json.RawMessage(`"ok"`)}, nil
	}
	<-ctx.Done()
