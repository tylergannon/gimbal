...
Research scope: three deliberately different interfaces that help with run discovery, history, spatial structure, or attention without suggesting that Gimble should become a workflow authoring tool or a generic analytics dashboard. Image URLs below are public, directly embeddable references; they are not downloaded into the repository.
