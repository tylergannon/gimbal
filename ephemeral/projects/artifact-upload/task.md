...
The deployment contract is that the selected bucket or artifact prefix is already publicly readable and has a lifecycle rule that removes artifacts after approximately 90 days. Anyone possessing a returned URL can read the artifact, so the command is not suitable for secrets or sensitive material. Gimble does not configure, verify, or alter that retention policy and does not expose a TTL flag, deletion command, listing command, or multi-file upload in this work.
