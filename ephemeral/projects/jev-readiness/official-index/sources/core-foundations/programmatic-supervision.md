# Jev as a programmatic supervision sensor for Gimbal

## Purpose

This leaf translates Jev's documented architecture and known failure modes into a bounded Gimbal supervision role. It distinguishes what Jev can continuously detect from what still requires Gimbal code, a human, or a reasoning/generative agent.

## Key concepts

- **The strongest fit is a semantic sensor inside Gimbal, not a supervisor agent replacement.** TypeSafe explicitly describes System One as “for building AI-powered software, not agents”: it neither generates code nor chooses its own next action. Code owns control flow and side effects; Jev supplies narrow common-sense judgments over unstructured data. [how to build with system one](https://docs.typesafe.ai/concepts/how-to-build-with-system-one.md)
...
- **Predeclared action space:** Jev works best when Gimbal already knows possible intervention classes; it is not a discoverer of arbitrary new coaching text.
...
- **No causal coaching claim is documented.** Detecting a trace pattern accurately does not show that the chosen intervention improves the run. Gimbal needs outcome experiments that distinguish detection quality from intervention efficacy.
- **No explanation or supporting-span field is returned.** The primitive response contains typed values and probabilities, not a rationale or source citation. If a human reviewer needs evidence, Gimbal must preserve the evaluated trace window and identify candidate evidence separately. [api](https://docs.typesafe.ai/api.md)
...
- **Self-consistency is a design claim, not a guarantee in this source set.** The guide points to a separate cookbook for evidence; evaluate repeated-run variance on the intended Gimbal rubric. [how to build with system one](https://docs.typesafe.ai/concepts/how-to-build-with-system-one.md)

## Task recipes

### Prototype a Gimbal “semantic tripwire”
...
2. Define a Choice whose options are actions Gimbal can actually take. Give each option contrastive coverage, exclusions, and examples. Use [advanced](https://docs.typesafe.ai/primitives/advanced.md) as the rubric pattern.
...
2. Maintain deterministic counters and chronology in Gimbal, then include their named textual summaries only when semantically relevant.
...
- No documented streaming/session API; the described contract is stateless request/response, so Gimbal must own event selection and state assembly.
- No native mechanism to learn from Gimbal's examples; Jev's weights are shared and not customer-fine-tuned. Learning requires request examples/rubrics or a downstream model.
