...
- `RetryPolicy` defaults to two retries after the initial attempt, exponential delay from 0.5 seconds capped at 5 seconds, 25% subtractive jitter, retryable HTTP statuses 408, 429, and all 5xx, respect for retry headers, retries for connection and timeout errors, and a 30-second total retry budget. [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)] [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)] [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)] [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)] [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)]
- Backoff doubles per attempt up to the cap; jitter randomly subtracts a configured fraction. A zero backoff disables delay. The total retry budget includes the initial attempt and delays, and the SDK stops before a delay that would reach or exceed it, re-raising the last error. [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)] [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)] [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)]
- Retry behavior is configurable at client level and overridable per call; `max_retries=0` disables retries. Invalid API keys fail during client creation before any request or retry. [[usage](https://docs.typesafe.ai/sdk/python/usage.md)]
- Beyond built-in HTTP/connection/timeout rules, a set of exception types and an exception predicate can opt additional failures into retrying. These are additive, so a permissive predicate can unintentionally retry deterministic validation/programming failures. [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)]
- All SDK failures derive from `TypeSafeError`. `TypeSafeAPIError` represents an unsuccessful HTTP response and retains status, parsed JSON or text body, headers, sanitized endpoint, and optional request ID. Specialized subclasses cover 400, 401, 403, 404, 422, 429, and 5xx. [[exceptions](https://docs.typesafe.ai/sdk/python/api/exceptions.md)] [[exceptions](https://docs.typesafe.ai/sdk/python/api/exceptions.md)] [[exceptions](https://docs.typesafe.ai/sdk/python/api/exceptions.md)]
- `TypeSafeRateLimitError.retry_after_ms` exposes the server-requested wait when present. RetryPolicy can honor `Retry-After` and `retry-after-ms`; the docs do not define precedence between those headers and exponential backoff. [[exceptions](https://docs.typesafe.ai/sdk/python/api/exceptions.md)] [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)]
- A connection error means no HTTP response. A timeout error is also a connection error and Python `TimeoutError`, carrying the float or `httpx2.Timeout` used. A response-validation error instead represents a successful HTTP response whose required body is missing/invalid and exposes a dotted offending `field_path`. [[exceptions](https://docs.typesafe.ai/sdk/python/api/exceptions.md)] [[exceptions](https://docs.typesafe.ai/sdk/python/api/exceptions.md)]
- Version history is unusually compressed: initial public release v0.5.7 on 2026-09-14, breaking Score criteria change plus type/error/pickle changes in v0.6.0 on 2026-09-15, serialization migration and `response_model` in v0.7.0 on 2026-09-18, then API-key/logging fixes and gateway examples in v0.7.1 on 2026-09-21. [[changelog](https://docs.typesafe.ai/sdk/python/changelog.md)]

## Important citation bookmarks

- Retry defaults in one constructor signature: [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)]
- Retryable status defaults: [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)]
- Retry headers and connection/timeout switches: [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)]
- HTTP error diagnostic fields: [[exceptions](https://docs.typesafe.ai/sdk/python/api/exceptions.md)]
- Transport versus response-validation failures: [[exceptions](https://docs.typesafe.ai/sdk/python/api/exceptions.md)]
- Full current changelog: [[changelog](https://docs.typesafe.ai/sdk/python/changelog.md)]
...
- The default can make up to three network attempts (initial plus two retries), not two total attempts. [[retries](https://docs.typesafe.ai/sdk/python/api/retries.md)]
...
- v0.6.0 changed `Score.criteria` from an integer-keyed dictionary to an ordered sequence; code or stored configs using the earlier shape are incompatible. [[changelog](https://docs.typesafe.ai/sdk/python/changelog.md)]
- v0.7.0 replaced `msgspec` with Pydantic and added `response_model`; serialization, equality, validation, or error details may differ across that boundary. [[changelog](https://docs.typesafe.ai/sdk/python/changelog.md)]
- v0.7.1's early key validation and exclusion from logged exceptions is security-relevant; do not assume earlier versions have the same safeguard. [[changelog](https://docs.typesafe.ai/sdk/python/changelog.md)]
