...
- This leaf extracts reusable delivery principles for Gimbal workflow authors and maintainers.
- The diffusion skills describe an external operating method. They do not, by themselves, define Gimbal builtins or a required public API.
...
- Gap: neither skill gives a Gimbal-level predicate for “materially unresolved”; authors still need judgment.
...
- Conflict: do not infer a universal task schema or a Gimbal wrapper from this design hypothesis; the document explicitly rejects an enormous universal document. [ephemeral/legacy/docs/workflows-as-programs.md:L119-L122]
...
- Conflict: exact model assignments and a fixed number of roles are staging choices, not Gimbal semantics.
...
- Conflict: session reuse is delivery orchestration behavior, not evidence that a Gimbal run should expose or depend on a vendor session identifier.
...
- Conflict: the sprint skill’s ledger updates and CLI preflight are process mechanics, not automatically runtime nodes in a Gimbal workflow.
...
- Gap: the skills do not prescribe the shape of a Gimbal proof artifact or live browser/runtime probe.
- Conflict: do not turn these reporting files into new Gimbal proof programs; they describe delivery practice, while Gimbal’s own proof contract must remain local to the workflow/package.

## What should not become a Gimbal builtin
...
- The durable Gimbal topics are therefore responsibility-aware context, explicit artifact handoffs, bounded routing, independent behavioral validation, and honest deferred findings; the exact diffusion staging remains an external method.
