...
- Audience: an author deciding how to package a Gimbal workflow for repeated use,
  ...
  Retrieval hints: `gimbal-workflows`, `gimbalgen`, `Command(defaults)`,
...
- A workflow is ordinary Go in a package under `internal/workflows/`; its entry
  has the shape `func Name(ctx context.Context, env gimbal.Env, params NameParams)
  error`, with workflow-owned arguments in a workflow-specific parameter type.
  [current instruction: `.agents/skills/gimbal-workflows/SKILL.md:123-129`]
- `gimbal.Env` is Gimbal-owned and carries the absolute initial `WorkDir`; an
...
- The generator directive names both the entry and the CLI workflow name. With
  ...
  `.agents/skills/gimbal-workflows/SKILL.md:123-137`]
...
- `describe` validates that the entry has `context.Context`, `gimbal.Env`, and at
...
- `gimbalgen` itself accepts only `-entry`, `-name`, and `-o`; `-entry` and
  `-name` are required and output defaults to `workflow_gen.go`. [current source:
  `internal/generate/gimbalgen/main.go:23-38`]
...
- The generated file declares `Graph`, emits `func init() { gimbal.RegisterGraph(Graph) }`,
...
- A built-in CLI must explicitly import the workflow package and add its generated
  command to `gimbal run`. The current binary does that for `review` in one line.
  [current source: `cmd/gimbal/workflows.go:7-10`, `cmd/gimbal/workflows.go:23-31`]
- The root CLI describes `run` as workflows built into this binary; the generated
  ...
  registered. [current source: `cmd/gimbal/workflows.go:23-29`; current source:
  `cmd/gimbal/main.go:112-119`]
- The generated `RunE` resolves the work directory, binds role models, creates a
  runtime, builds `gimbal.Env`, and invokes the entry. [current generator:
...
- The authoring skill should tell authors to make prompts, role names, and command
  ...
  instruction: `.agents/skills/gimbal-workflows/SKILL.md:146-150`; current design:
...
- Existing CLI tests prove flag presence, requiredness, and model defaults for
  ...
  examples. [current test: `cmd/gimbal/workflows_test.go:23-66`]
...
- Gap: no generated help field describes outputs. Result type comments and JSON
  ...
  current instruction: `.agents/skills/gimbal-workflows/SKILL.md:53-60`]
- Gap: no generated help field describes proof or observation. Runtime records
  ...
  counts as demonstrated. [current instruction: `.agents/skills/gimbal-workflows/SKILL.md:146-150`;
...
- Gap: no generated help field offers a runnable example. The authoring skill
  ...
  `.agents/skills/gimbal-workflows/SKILL.md:139-144`; current generator:
...
- Gap: there is no generator lint requiring the six documentation topics above,
  ...
  workflows are well documented via CLI. [current test: `cmd/gimbal/workflows_test.go:23-66`;
  proposal: this leaf]
- Portability friction: generated commands import `internal/binding` and `web`
  ...
  a Gimbal repository/module layout. [current generator:
  `internal/generate/command.go:102-115`]
- Portability friction: the generator identifies Gimbal calls by the exact module
  path `github.com/tylergannon/gimbal`, and package loading expects a Go package
...
- Portability friction: generated `init` graph registration is usable wherever
  ...
  source: `internal/generate/command.go:118-124`; `cmd/gimbal/workflows.go:23-31`]
- Build friction: `go generate` needs the module's registered tools and Polytype
  ...
  `.agents/skills/gimbal-workflows/SKILL.md:53-60`]
...
- For each built-in, prove `gimbal run --help` and `gimbal run <name> --help`,
  ...
  instruction: `.agents/skills/gimbal-workflows/SKILL.md:139-150`; proposal: this
  leaf]
