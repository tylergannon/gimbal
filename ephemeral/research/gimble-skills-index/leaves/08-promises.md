...
- **Audience:** workflow authors, builders/releasers of Gimble, and people using
  Gimble to make agent work observable and steerable.
...
- **Use this leaf for:** teaching what a promise means, how proof earns or
  ...
  ordinary Go workflow without treating the helper as a Gimble builtin.
- **Source boundary:** `df-promise` is a skill plus Python helper. These
  sources do not establish it as a Gimble API or runtime primitive.
...
- Use Gimble’s normal workflow structure to make these bounds visible at the
...
## What this means for Gimble workflows
...
- Historical direction treats telemetry for bad runs as a proposed information
  ...
  shipped Gimble promise features. [ephemeral/legacy/docs/direction.md:244-250; ephemeral/legacy/docs/direction.md:330-342]
...
- The Gimble repository’s contribution contract forbids committing run/proof artifacts.
  This restriction concerns work in this repository; other consumer projects
  retain their own artifact policies. In Gimble itself, do not import `.promises/`, badges, run logs, or proof programs unless
...
- The badge is historical and commit-pinned, not a self-refreshing CI signal.
  A user of Gimble must say what was actually run and observed, with model,
