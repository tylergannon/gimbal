...
## Beta implementation prerequisite — graph recording first

Tyler's sequencing decision, September 14: do not start building this viewer until the workflow graph shape is being recorded with runs. The order is #201 (extract the graph), then #215 (preserve it with each run and serve it through observation), then this viewer. Earlier suggestions to build the viewer alongside extraction are superseded. Use the recorded graph from #215 as the program-shape input; an observed-only hierarchy is not the intended final view.
