...
This is a simplicity preference, not a requirement to predict the run or prove arbitrary Go safe. Each authoring diagnostic needs a recognizable forbidden construct, a simpler permitted example, and a bounded implementation. An unsupported but straightforward construct is an analyzer gap, not automatically an authoring violation. Lint errors fail the linter/gate; Go compilation and direct execution remain available.

## Rule candidates

Only dynamic-worker dispatch has been newly approved in this discussion. The remaining rows are recommendations for review, grounded in current API/design rules where noted; they are not new acceptance requirements merely because they appear here.
...
Flag known missing joins on ordinary analyzed return paths. Do not turn “could not prove Wait through an arbitrary helper” into an error. General panic/recover, escape, and every-path lifetime proofs are outside the initial check. Existing wrong-context checks must also accept ordinary cancellation/deadline derivations of the correct scope context.
...
- A blanket ban on function parameters, interfaces, maps, loops, dynamic command arguments, model/provider choice, or dynamic Set keys.
...
Can a reader see the possible workflow shape in the permitted rewrite, and can the analyzer recognize the violation without speculative whole-program reasoning? If the rule only compensates for an implementation shortcut, improve or limit the analyzer instead. If the rewrite clarifies orchestration, the rule is a useful Beta candidate.
