...
// functions import gimbal and gimbal cannot import the page.
...
func main() {
	...
	defer stop()
	runtime, err := web.NewRuntime(ctx, ".gimbal")
	if err != nil {
		log.Fatal(err)
	}
...
- `web.Runtime` directly owns the project and assembled web application; the
  root `gimbal` package does not import or expose web assembly. Workflow tests
