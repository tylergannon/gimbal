...
Represent **declaration ownership**: the root owns root services, a named child owns its declarations, and an iteration owns one static declaration set in its reusable body. Keep constant names and source locations. [Issue #284](corpus/topic-001/sources/issue-284.md) requests this before execution; [#276](corpus/topic-001/sources/issue-276.md) and the [settled service contract](corpus/topic-001/sources/gimbal-api.md#L988) establish the lifetime boundary without requiring lifetime data in the graph.
...
| [GitHub Actions services](corpus/topic-007/sources/github-actions-docker-services.md#L16) | Named services belong to a job; this is one ownership level, not arbitrary nested scopes. | Closest structural precedent: `jobs.<job>.services` beside steps. Does not establish service declaration order or Gimbal source identity. |
...
None of these cited models supplies Gimbal’s constant call-site name plus module-relative file/line contract. Retain that contract directly rather than adopting foreign IDs. Iteration semantics come from Gimbal’s static body contract, not these products’ resource instances.
