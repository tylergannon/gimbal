...
| Descendants | The guarantee covers descendants that remain in the deliberately owned process-group/session boundary. A nested tool that escapes that boundary or daemonizes is outside Gimble's resource management, but escape or inability to verify cleanup must be visible as incomplete cleanup, not hidden. |
