...
Currently, the CLI sends an untyped `Models map[gimble.WorkflowRole]string` on the wire (`internal/generate/command.go:209`).
...
2. **Distinguishing default inheritance from explicit overrides**:
   ...
   - On the server, `req.RoleCoding.Present` is `false`. The server falls back to `defaults[gimble.WorkflowRole("coding")]` loaded from `defaults.json`.
...
1. **Resolution pipeline**:
   ...
   - Call `internal/binding.Roles(specs)`:
     ...
     - Constructs `gimble.ModelBinding{Adapter: adapter, Model: resolved.Model, Effort: resolved.Effort}`.
