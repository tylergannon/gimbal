...
- [gimble-ui-conventions-and-dos-donts.md](./sources/gimble-ui-conventions-and-dos-donts.md): Owner's hard rules in `ephemeral/research/svelte-idioms/dos-and-donts.md`, web app features in `docs/web-app.md`, and shadcn-svelte primitives (`web/src/lib/components/ui/input/input.svelte`).
- [gimble-route-hierarchy-and-middleware.md](./sources/gimble-route-hierarchy-and-middleware.md): Route layout in `web/src/routes/+layout.svelte`, root projects page `web/src/routes/+page.svelte`, and project-scoping middleware in `web/runtime.go`.
...
- **Supported Facts**:
  - Existing route structure in `web/src/routes/` is:
    - `/` (`routes/+page.svelte`): instance root, lists admitted projects ([`gimble-route-hierarchy-and-middleware.md:3`](./sources/gimble-route-hierarchy-and-middleware.md#3-instance-root-projects-page)).
  ...
  - Owner's hard rule forbids query parameters for identity or page state ([`gimble-ui-conventions-and-dos-donts.md:1`](./sources/gimble-ui-conventions-and-dos-donts.md#1-owners-hard-rules-for-svelte-and-sveltekit)). Identity must be expressed as path segments.
  - Start remotes are instance-scoped and admit arbitrary directories not previously known to the host ([`implementation-plan.md:138-144`](../../implementation-plan.md#3-generate-concrete-starts-and-admit-projects-on-first-use)). Under `web/runtime.go:391`, the existing middleware intercepts `/projects/` and `/remote/` and returns 404 unless a matching admitted project ID is found in the path or Referer ([`gimble-route-hierarchy-and-middleware.md:1`](./sources/gimble-route-hierarchy-and-middleware.md#1-project-scoping-middleware-in-webruntimego)).
...
- **Supported Facts**:
  ...
  - `ephemeral/research/svelte-idioms/dos-and-donts.md` rule #10 explicitly identifies hidden proxy forms as a severe anti-pattern, requiring real `<form {...formRemote}>` bindings ([`gimble-ui-conventions-and-dos-donts.md:2`](./sources/gimble-ui-conventions-and-dos-donts.md#2-prohibition-of-hidden-proxy-forms-rule-10)).
  - SvelteKit 3 form runtime (`form.svelte.js` and `form-utils.js`) provides:
    ...
    - **Shadcn Integration**: Shadcn `Input` (`web/src/lib/components/ui/input/input.svelte`) styles `aria-invalid` natively with `aria-invalid:border-destructive aria-invalid:ring-destructive/20` ([`gimble-ui-conventions-and-dos-donts.md:5`](./sources/gimble-ui-conventions-and-dos-donts.md#5-built-in-aria-invalid-support-in-shadcn-svelte-input)).
...
- **Supported Facts**:
  - Server context projects `data.projects` (`ProjectChoice{ID, Path}`) into load functions via `hooks.Projects(ctx)` ([`gimble-route-hierarchy-and-middleware.md:4`](./sources/gimble-route-hierarchy-and-middleware.md#4-context-projection-types-in-websrcprojectgo)).
  - Model defaults are structured per-role in `cmd/gimble/defaults.json` (`gpt-6-luna`, `claude-opus-5-5`, `gemini-3.8-flash-medium`, etc.) ([`cmd/gimble/defaults.json`](../../../../../cmd/gimble/defaults.json)).
  - SvelteKit navigation must not use raw History API; it must use `goto` from `$app/navigation` ([`gimble-ui-conventions-and-dos-donts.md:1`](./sources/gimble-ui-conventions-and-dos-donts.md#1-owners-hard-rules-for-svelte-and-sveltekit)).
