...
- [gimble-test-matrix-and-e2e.md](./sources/gimble-test-matrix-and-e2e.md): Existing test runners (`Justfile`), Playwright BDD browser test harness (`e2e/playwright.config.ts`), and comparison of legacy in-process direct calls vs real client protocol tests.
...
- [cross-repo-sequencing-and-pinning.md](./sources/cross-repo-sequencing-and-pinning.md): Module dependencies in `go.mod`, SKGO client generation, and delivery rules in `skills/gimble-release/SKILL.md`.
- [test-matrix-clip.md](./clips/test-matrix-clip.md): Structured test layer matrix spanning SKGO unit fixtures, Gimble package HTTP/UDS tests, and Playwright E2E.

---

## Question Routing and Local Evidence

### Q1: How should existing tests in Gimble (just test, just e2e) be updated or extended to exercise both browser and Go client paths against the same remote handler without mocks?

- **Supported Facts**:
  - The definition of done establishes: *"A direct call to a shared Go function alone does not prove the client/remote path"* ([`gimble-test-matrix-and-e2e.md:3`](./sources/gimble-test-matrix-and-e2e.md#3-direct-function-calls-vs-client-protocol-proof)).
  - Existing tests like `web/interview_test.go` directly invoked `routes.Skgo_answerInterview` in-process, bypassing the HTTP wire protocol, header validation, and error envelopes ([`gimble-test-matrix-and-e2e.md:3`](./sources/gimble-test-matrix-and-e2e.md#3-direct-function-calls-vs-client-protocol-proof)).
  - Existing tests like `web/submit_test.go` exercised `/control/submit` with generic `Submission{}` payloads, which is slated for deletion ([`gimble-test-matrix-and-e2e.md:4`](./sources/gimble-test-matrix-and-e2e.md#4-retaining-run-ownership-beyond-client-lifecycle)).
  - Playwright E2E (`e2e/playwright.config.ts`) runs against the real embedded production binary `bin/gimble --port 0` without mocks ([`gimble-test-matrix-and-e2e.md:2`](./sources/gimble-test-matrix-and-e2e.md#2-real-binary-web-server-in-playwright-e2e)).
- **Inference & Recommendation**:
  - **`just test` (Package tests in `web/` and `cmd/gimble`)**:
...
- **Inference & Recommendation**:
  - **Verification Scenario 1: Multi-Project Concurrency**:
    1. Start a single server instance `bin/gimble --port 8080` (PID $PID).
    2. Admitting Project A via CLI `gimble review --target HEAD~1` (binding `code-review` to `gpt-5.6-luna`).
    3. Concurrently admit previously unknown Project B via visible browser start form at `http://127.0.0.1:8080/start/implement`.
    4. Confirm single PID $PID serves both, active runs appear in their respective `.gimble/runs/` directories, and Project A's interface cannot access Project B's runs.
  - **Verification Scenario 2: Client Disconnect Survival**:
    ...
    3. Reopen browser or attach with `gimble follow <runID>` to observe terminal completion.
  - **Verification Scenario 3: Input Rejection**:
    1. Submit a start request with missing required fields or invalid model name.
    2. Observe immediate HTTP 400 / field issues returned; confirm no run directory or ID was created under `.gimble/runs/`.
- **Unresolved / Decision Points**:
  - Exact model name alias configuration for test runs (verifying `cmd/gimble/defaults.json` maps each role to the cheap tier during test invocations).

---

### Q3: What exact cross-repository workflow (local branch testing, SKGO release tagging, Gimble module pinning, and post-install validation) ensures delivery without breaking CI or leaving local replacements?

- **Supported Facts**:
  - Gimble uses Go modules with a strict pinning policy: `github.com/tylergannon/skgo v0.5.0` in `go.mod` ([`cross-repo-sequencing-and-pinning.md:1`](./sources/cross-repo-sequencing-and-pinning.md#1-pinned-dependency-structure-in-gomod)).
  ...
  - Plan and DoD rule: *"Land and publish the needed SKGO change, then pin that version in Gimble; leave no local module replacement in the delivered build"* ([`cross-repo-sequencing-and-pinning.md:2`](./sources/cross-repo-sequencing-and-pinning.md#2-cross-repository-sequencing-rules)).
  - `skills/gimble-release/SKILL.md` requires clean checkout build with `just build` and `go install ./cmd/gimble` without local replacements ([`cross-repo-sequencing-and-pinning.md:3`](./sources/cross-repo-sequencing-and-pinning.md#3-merge-installation-and-clean-build-standards)).
- **Inference & Recommendation**:
  ...
  - **Phase 2 (Local Integration in Gimble Worktree)**:
    - Temporarily add `replace github.com/tylergannon/skgo => ../../skgo` in Gimble's `go.mod` for local integration and generator debugging (`go generate ./...`, `just test`, `just e2e`).
  - **Phase 3 (SKGO Release Tagging)**:
    - Land SKGO changes on its main branch, tag semantic release (e.g. `v0.6.0`), and push to GitHub.
  - **Phase 4 (Gimble Module Pinning & Verification)**:
    - Remove the `replace` directive from Gimble's `go.mod`.
  ...
  - **Phase 5 (Post-Install Validation)**:
    - Install CLI locally: `go install ./cmd/gimble`.
    - Reinstall skills: `skills add ...` per `skills/gimble-release/SKILL.md`.
    - Run `gimble --help` and verify installed binary serves web app and executes workflow remotes.
- **Unresolved / Decision Points**:
