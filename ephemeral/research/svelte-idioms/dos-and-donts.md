...
The rule: **if Svelte or SvelteKit has a high-level API or paradigm for it, use it.** Each entry is an anti-pattern seen in `web/` (main at `aeba1bf`, 2026-09-21), the replacement, the doc section that backs it (line numbers are in `svelte-idiomatic.txt`, a local docs subset described in `docs-subset.md`), a grep signature for finding it mechanically, and the sites. "n/10" is how many of a round's ten reviewers raised it independently (entries 0–18 from round 1, 19–31 from round 2). Raw reports: `round-1/`, `round-2/` (kept locally, not committed: agent output). Doc line numbers: in `svelte-idiomatic.txt` the "declaration tags" section (`{let/const ...}`) was restored after line 2442 on 2026-09-21, so cited lines above 2442 are now 71 higher; the section headings are the stable reference.
