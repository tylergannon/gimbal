...
func main() {
	...
	err = runtime.Run(ctx, "mergequeue-review", func(ctx context.Context) error {
		if err := gimble.Set(ctx, "requirements", string(goal)); err != nil {
			return err
		}
		if err := gimble.Set(ctx, "role", "Plan work; edit only the runtime backlog."); err != nil {
			return err
		}
		if err := gimble.Set(ctx, "constraints", "Use only the standard library. Work in the isolated fixture. Do not commit or access GitHub. Acceptance is external and cannot be changed by agents."); err != nil {
			return err
		}
		...
		for taskCtx, task := range loop.Tasks {
			...
			if err := gimble.Set(taskCtx, "role", "Implement the selected task and demonstrate it; leave changes uncommitted."); err != nil {
				return err
			}
			...
			group.Go("commands", func(checkCtx context.Context) error {
				...
				return gimble.Set(checkCtx, "command evidence", acceptance+"\n"+checks)
			})
			group.Go("independent", func(reviewCtx context.Context) error {
				if err := gimble.Set(reviewCtx, "role", "Read-only independent validator; assess the requested behavior, tests and README. Do not edit, commit, or access GitHub."); err != nil {
					return err
				}
				...
				return gimble.Set(reviewCtx, "assessment", assessment)
			})
			if err := group.Wait(); err != nil {
				return err
			}
			// Descendant scope values are not included in Loop's next prompt: promote selected results explicitly.
			if err := gimble.Set(taskCtx, "acceptance evidence", acceptance); err != nil {
				return err
			}
			if err := gimble.Set(taskCtx, "repository checks", checks); err != nil {
				return err
			}
			if err := gimble.Set(taskCtx, "independent assessment", assessment); err != nil {
				return err
			}
			...
			if !extended && acceptanceCode == 0 {
				...
				if err := gimble.Set(taskCtx, "new requirement", newRequirement); err != nil {
					return err
				}
				if err := gimble.Set(taskCtx, "new requirement evidence", next); err != nil {
					return err
				}
		...
		for _, file := range []string{"acceptance.py", "parallel.py"} {
			...
			if err := gimble.Set(ctx, file, out); err != nil {
				return err
			}
			if code != 0 {
