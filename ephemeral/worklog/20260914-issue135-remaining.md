...
decision: retain completed live-run files through `GIMBLE_LIVE_PROJECT` so the existing production handler can render the exact reconnect transcript after the live-gated test exits; this adds no production API or validation framework.
