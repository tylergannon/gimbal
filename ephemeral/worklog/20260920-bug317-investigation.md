...
correction: User wanted Gimbal with lower models to do the heavy lifting. Direct df-sprint-plan draft/critique CLI launches used Fable/Astra/default Gemini and missed that routing intent. No harness changes were made. Remaining plan synthesis and independent validation were assigned through Gimbal to Terra and Sol, with an explicit documentation-only boundary. User routing overrides skill defaults.

decision: User expressly requested a temporary acceptance workflow. It lives outside the checkout at /private/tmp/gimbal-317-sprint/ and exercises the real public Run/Session/Generate path with Haiku. Its current-harness baseline returned a typed waiting response before background completion and failed EARLY_RETURN. Sprint completion must require the same two-call scenario to pass after implementation; planning does not itself authorize the repair.

decision: Gimbal run 01M2ZXA75D0Z3TY09HAMJR98DS.implement finished successfully using Terra for planning/drafting/critique and Sol for independent validation. The assignment owned only Sprint 002 and its merge notes. Sol validated the documents against the existing fixture and raw red baseline, with no gaps; this does not satisfy the future implementation gate. Sprint status remains planned.

correction: User authorized implementing Sprint 002 through Gimbal, then explicitly selected Sol medium for coding and Sol high for review. No Terra implementation run was launched. Planning and architectural critique also use Sol high; Claude Haiku is reserved for the required adapter acceptance.

correction: Live captures xwz40a9n and p_314klf exposed a fixture contradiction: completed native task_notification supplied output_file plus an exit summary, not stdout, while the probe demanded an unpredictable stdout receipt and banned Read. Root corrected only this restriction: one Read of the exact matching notification output_file after notification is allowed. No pre-notification reads, TaskOutput, host reprompts, extra work, or weakening of waiting/return/schema checks. Existing runs remain recorded failures; a fresh run must pass the corrected fixture.

decision: Implementation run 01M2ZZA2GYPG74DHMR6VE76VWG.implement used Sol medium coding and Sol high planning/critique/independent validation. The validator personally passed the corrected Haiku acceptance plus full Go/frontend checks and Claude race checks, with no substantial gaps. Private completion keeps native waiting continuations alive; a native init/requesting barrier excludes pre-prompt orphan results. Legacy dependencies name-map handling was explicitly deferred to issue 321 rather than expanding this repair.
