...
decision: User reversed async-only to simplify initial delivery: synchronous prompt POST plus shared SSE, with raw event capture and response correlation. Later Luna analysis may justify event-only reduction; do not discard POST authority yet. User explicitly requested Gimbal workflows for implementation.
...
decision: User chose the legacy API for the adapter. Keep one shared OpenCode server across projects/runs, controlled by `gimbal opencode start|stop`, with runtime-configurable state defaulting to `~/.gimbal/`. Verify source and settle remaining questions before implementation. Generate only required types; handwritten HTTP methods and SSE are preferred over further SDK codegen machinery.

correction: Legacy `/event` is directory/workspace-scoped; `/global/event` spans the server. The existing Gimbal event callback already records correct run ownership, so a new adapter needs session routing, not another storage layer.
...
correction: Gimbal steering requires delivery into the active RunTurn. Consumption at the next model/tool boundary within that same turn can qualify; mid-token provider interruption is not required.
...
friction: Links from ephemeral/research/opencode to the local .gimbal corpus require three parent levels. Check report reference targets after synthesis.
...
friction: Reusing the corpus did not avoid agy failure: retry run01M2XMQ94EEC1Y3XQ7CBAYP5GY.research-document ended during research with result arriving before tool step30 settled. Requested role-model replacement per gimbal-runs instruction; no application repair in this research scope.
