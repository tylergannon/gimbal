...
func TestRolesShareABindingAndNameTheMissingFlag(t *testing.T) {
	models, err := Roles(map[string]string{"coder": "gpt-5.6-luna", "planner": "gpt-5.6-luna", "reviewer": "claude-haiku-4-5-20251001"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := Roles(map[string]string{"coder": "gpt-5.6-luna", "planner": ""}); err == nil || !strings.Contains(err.Error(), "give --planner") {
