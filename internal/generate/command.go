...
func check(info entryInfo, graph workflow.Graph) error {
	...
	for role := range info.defaults {
		if !slices.Contains(roles, role) {
			return fmt.Errorf("generate: roles gives %s a model, but %s creates no session for that role", role, graph.Name)
		}
	}
	return nil
}
...
var commandTemplate = template.Must(template.New("command").Parse(`// Code generated by internal/generate. DO NOT EDIT.
