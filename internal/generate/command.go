...
// integrationSource prints the application integration outside the workflow
// package: its hosted adapter and gimble run subcommand, with Gimble's environment
...
func integrationSource(pkg, entry string, info entryInfo, graph workflow.Graph) string {
	data := commandData{Package: pkg, Name: graph.Name, Entry: entry, Params: info.params, Summary: info.summary, Long: info.long, Ref: identifier(pkg), Function: identifier(pkg)}
...
import (
	...
	{{.Ref}} "github.com/tylergannon/gimble/internal/workflows/{{.Package}}"
...
// {{.Function}}Hosted is the entry {{.Entry}} supplies to the persistent instance.
func {{.Function}}Hosted() web.WorkflowEntry {
	return func(ctx context.Context, env gimble.Env, raw json.RawMessage) error {
		{{if .Params}}var params {{.Ref}}.{{.Params}}
		if err := json.Unmarshal(raw, &params); err != nil { return err }
		return {{.Ref}}.{{.Entry}}(ctx, env, params){{else}}return {{.Ref}}.{{.Entry}}(ctx, env){{end}}
	}
}

// {{.Function}}Command is gimble run {{.Name}}: {{if .Params}}a flag for each field of {{.Params}}, {{end}}a
