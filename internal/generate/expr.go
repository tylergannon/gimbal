	...
	ast.Inspect(node, func(n ast.Node) bool {
		...
		case *ast.FuncLit:
			if e.holdsOperation(n.Body) {
				e.diag(n.Pos(), "a function literal that is not a Scope or Go callback holding a Gimbal call is not read")
			}
			return false
		case *ast.CallExpr:
...
func (e *extractor) call(call *ast.CallExpr, targets []ast.Expr, out *[]workflow.Operation, en scopeEnv) bool {
	if name, ok := e.gimbalCall(call); ok {
		return e.gimbalOperation(name, call, targets, out, en)
	}
	...
	if e.importsGimbal(callee.Pkg()) {
		e.diag(call.Pos(), "a call into %s, which imports Gimbal, is not followed", callee.Pkg().Path())
	}
	return true
}
...
// gimbalOperation reads one call on the Gimbal package. It reports whether
// the walk should keep descending into the call's own parts.
func (e *extractor) gimbalOperation(name string, call *ast.CallExpr, targets []ast.Expr, out *[]workflow.Operation, en scopeEnv) bool {
	e.nested(call.Pos(), "a Gimbal call's arguments", plainArguments(name, call)...)
	switch name {
	case "NewSession":
...
func (e *extractor) supervisor(option ast.Expr) (workflow.Supervisor, bool) {
	...
	if !ok {
		e.diag(option.Pos(), "an option that is not a Gimbal option is not read")
