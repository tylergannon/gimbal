...
func TestFixtureGraph(t *testing.T) {
	...
	// A Gimbal call inside another call's arguments is a diagnostic, not a node.
	...
	want := []string{
		...
		"a Gimbal call nested in a Gimbal call's arguments is not read",
	}
...
func TestSourceWritesTheWorkflowsPackage(t *testing.T) {
	...
	if strings.Contains(string(written), "github.com/spf13/cobra") || strings.Contains(string(written), "github.com/tylergannon/gimbal/web") {
...
func TestGeneratedEntryRequiresEnv(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "second parameter is not gimbal.Env") {
...
func TestWorkflowParamsCannotClaimWorkDir(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "parameter field WorkDir would be --work-dir, which is the Gimbal environment's") {
