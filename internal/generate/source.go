...
func writeMermaid(dir, name, output string, graph workflow.Graph) error {
	...
	var promptSource strings.Builder
	promptSource.WriteString("[")
	for _, prompt := range diagramPrompts(graph) {
		fmt.Fprintf(&promptSource, "\n    { role: %s, text: %s },", strconv.Quote(prompt.Role), strconv.Quote(prompt.Text))
	}
	if promptSource.Len() > 1 {
		promptSource.WriteString("\n  ")
	}
	promptSource.WriteString("]")
...
<WorkflowDiagram title=%s {src} {prompts} />
`, filepath.Base(strings.TrimSuffix(output, ".mmd")), promptSource.String(), strconv.Quote(name+" workflow"))
...
type diagramPrompt struct {
	Role string
	Text string
}

func diagramPrompts(graph workflow.Graph) []diagramPrompt {
	var prompts []diagramPrompt
	seen := map[string]bool{}
	var collect func([]workflow.Operation)
	collect = func(operations []workflow.Operation) {
		for _, operation := range operations {
			...
			case workflow.AgentCall:
				...
				key := role + "\x00" + operation.Prompt
				if operation.Prompt != "" && !seen[key] {
					seen[key] = true
					prompts = append(prompts, diagramPrompt{Role: role, Text: operation.Prompt})
				}
			case workflow.Scope:
