// Package gimballint checks deterministic workflow authoring mistakes.
package gimballint
...
const cmdPath = gimbalPath + "/cmd/gimbal"

const gimbalPath = "github.com/tylergannon/gimbal"

// rules is the canonical user-facing description of the analyzer. Embedding
// it makes `gimbal lint -help` describe the exact rules carried by the binary.
//
//go:embed rules.md
var rules string

// Analyzer checks the accepted Gimbal workflow lint rules. Its worker-dispatch
...
func indexSyntax(pass *analysis.Pass, in *inspector.Inspector) *syntaxInfo {
	...
	for _, file := range pass.Files {
		ast.Inspect(file, func(n ast.Node) bool {
			switch n := n.(type) {
			case *ast.CallExpr:
				name, ok := gimbalCall(pass, n)
				if ok {
					if u := enclosingUnit(si, n); u != nil && pass.Pkg.Path() != gimbalPath && isWorkflowOperation(name) {
						u.workflow = true
					}
				...
				if callee := typeutil.StaticCallee(pass.TypesInfo, n); callee != nil && callee.Pkg() == pass.Pkg {
					// In same-package tests, calls to Gimbal's API resolve as local
					// functions. They are workflow nodes, not helper bodies to follow
					// into the runtime implementation.
					apiCall := pass.Pkg.Path() == gimbalPath && isWorkflowOperation(callee.Name())
...
// compile-time string constant (GIMBAL108), and a WithScopeTemplate template
// that is neither that nor an embedded file (GIMBAL109). Package cmd is
...
func isSSASet(call ssa.CallInstruction) bool {
	...
	return callee != nil && callee.Pkg != nil && callee.Pkg.Pkg.Path() == gimbalPath && (callee.Name() == "Set" || callee.Name() == "SetJSON" || callee.Name() == "Check")
}
...
func gimbalCall(pass *analysis.Pass, call *ast.CallExpr) (string, bool) {
	callee := typeutil.Callee(pass.TypesInfo, call)
	if callee == nil || callee.Pkg() == nil || callee.Pkg().Path() != gimbalPath {
		return "", false
	}
...
func scopedRange(pass *analysis.Pass, stmt *ast.RangeStmt) (tasks bool, ok bool) {
	...
	if call, isCall := expr.(*ast.CallExpr); isCall {
		name, isGimbal := gimbalCall(pass, call)
		return false, isGimbal && name == "Iterate"
	}
	...
	if selection != nil {
		obj := selection.Obj()
		if obj.Pkg() != nil && obj.Pkg().Path() == gimbalPath && obj.Name() == "Tasks" {
			return true, true
		}
	}
	obj := pass.TypesInfo.Uses[sel.Sel]
	if obj != nil && obj.Pkg() != nil && obj.Pkg().Path() == gimbalPath && obj.Name() == "Tasks" {
		return true, true
	}
	return false, false
}
