...
// Source writes the graph of the workflow entry in dir as a Go file holding
// one typed literal, which its init registers, so a workflow is wired to
// its own shape by being built.
//
// The file it writes is replaced by an empty package clause while the package
...
func Source(dir, entry, name, output string) error {
	...
	text, err := format.Source([]byte(literal(pkg, entry, info, graph)))
	if err != nil {
		return fmt.Errorf("graph: %w", err)
	}
...
func literal(pkg, entry string, info entryInfo, graph workflow.Graph) string {
	var b strings.Builder
	fmt.Fprintf(&b, "// Code generated by gimble graph. DO NOT EDIT.\n\npackage %s\n\n", pkg)
	b.WriteString("import (\n\"context\"\n\"encoding/json\"\n\n\"github.com/tylergannon/gimble\"\n\"github.com/tylergannon/gimble/workflow\"\n)\n\n")
	b.WriteString("// The workflow registers itself when it is built: gimble lists it and runs it.\n")
	b.WriteString("func init() {\ngimble.RegisterWorkflow(gimble.Registration{\n")
	fmt.Fprintf(&b, "Name: %s,\n", strconv.Quote(graph.Name))
	fmt.Fprintf(&b, "Summary: %s,\n", strconv.Quote(info.summary))
	if info.input != "" {
		fmt.Fprintf(&b, "Input: (%s{}).Schema(),\n", info.input)
	}
	b.WriteString("Graph: Graph,\n")
	if info.input != "" {
		fmt.Fprintf(&b, "Run: func(ctx context.Context, input json.RawMessage) error {\nvar in %s\nif err := in.ValidateJSON(input); err != nil {\nreturn err\n}\nif err := json.Unmarshal(input, &in); err != nil {\nreturn err\n}\nreturn %s(ctx, in)\n},\n", info.input, entry)
	} else {
		fmt.Fprintf(&b, "Run: func(ctx context.Context, _ json.RawMessage) error { return %s(ctx) },\n", entry)
	}
	b.WriteString("})\n}\n\n")
