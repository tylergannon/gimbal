...
func TestToolProgressAfterCompletionRetainsNestedTranscript(t *testing.T) {
	...
	for _, event := range []*Obj{
		...
		nativeEvent("success", "session.tool.success", "sessionID", "ses", "assistantMessageID", "message", "id", "tool", "content", []any{obj("type", "text", "text", "launched")}, "executed", true),
		nativeEvent("progress", "session.tool.progress", "sessionID", "ses", "assistantMessageID", "message", "id", "tool", "metadata", obj("transcript", []any{obj("type", "session.text.delta")})),
	} {
	...
	requireJSON(t, "completed tool transcript", objOf(tool.Get("state")).Get("metadata"), `{"transcript":[{"type":"session.text.delta"}]}`)
}
