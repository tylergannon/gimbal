...
import (
	...
	skgo3 "github.com/tylergannon/gimble/internal/skgo/links/onzggl3sn52xizltf5rw63twmvzhgylunfxw44zplnrw63twmvzhgylunfxw4skelu"
	skgo4 "github.com/tylergannon/gimble/internal/skgo/links/onzggl3sn52xizltf5zhk3ttf5nxe5lojfcf2"
...
// remote_createConversation answers src/routes/conversation.remote.ts#createConversation, a form.
//
...
func remote_createConversation(ctx context.Context, call skgo.Call) (any, error) {
	var in skgo1.SkgoArg_createConversation
...
// remote_sendConversationMessage answers src/routes/conversation.remote.ts#sendConversationMessage, a form.
//
// A form's submission is assigned onto the handler's own argument type:
// kit posts a form as binary form data, which can carry an uploaded File,
// and a File is not a value polytype describes.
...
func remote_sendConversationMessage(ctx context.Context, call skgo.Call) (any, error) {
	var in skgo1.SkgoArg_sendConversationMessage
	if err := skgo.DecodeForm(call.Arg, &in); err != nil {
		return nil, err
	}
...
// remote_watchConversation answers src/routes/conversation.remote.ts#watchConversation, a query.live.
//
// The argument is decoded by DecodeRoot4, generated by polytype for watchConversation's
// own parameter type: a shape it does not admit is a 400 naming the path,
// never a zero value.
// The result is encoded by EncodeRoot3, generated for its result type.
func remote_watchConversation(ctx context.Context, call skgo.Call, yield func(any) error) error {
	if err := skgo.RequireArgument(call); err != nil {
		return err
	}
	...
	return skgo1.Skgo_watchConversation(ctx, in, func(out skgo1.SkgoOut_watchConversation) error {
...
// requestedArg_watchConversation decodes one instance's argument for skgo.Requested, with the same
// decoder remote_watchConversation uses.
func requestedArg_watchConversation(arg any) (any, error) {
...
// remote_watchRuns answers src/routes/runs.remote.ts#watchRuns, a query.live.
//
...
func remote_watchRuns(ctx context.Context, call skgo.Call, yield func(any) error) error {
	if err := skgo.RefuseArgument(call); err != nil {
		return err
	}
	return skgo1.Skgo_watchRuns(ctx, func(out skgo1.SkgoOut_watchRuns) error {
...
func Remotes() []*skgo.Remote {
	return []*skgo.Remote{
		...
		skgo.NewRemote(skgo.RemoteSpec{
			...
			Fn:        skgo1.Skgo_watchConversation,
			Live:      remote_watchConversation,
			DecodeArg: requestedArg_watchConversation,
		}),
		skgo.NewRemote(skgo.RemoteSpec{
			Kind:   skgo.KindForm,
		...
		skgo.NewRemote(skgo.RemoteSpec{
			Kind:   skgo.KindLive,
			Module: "src/routes/runs.remote.ts",
			Name:   "watchRuns",
			Fn:     skgo1.Skgo_watchRuns,
			Live:   remote_watchRuns,
		}),
		skgo.NewRemote(skgo.RemoteSpec{
			Kind:   skgo.KindForm,
...
// load_conversations_layout answers src/routes/conversations/+layout.server.ts.
//
...
func load_conversations_layout(ctx context.Context) (any, error) {
	return skgo2.Skgo_load(ctx)
}

// load_conversations_conversationID_page answers src/routes/conversations/[conversationID]/+page.server.ts.
//
// A load's result is the one value no generated encoder produces: it may
// hold a skgo.Deferred, and `Promise<T>` is not a projection of any Go
// type, so the value is encoded where a promise can still be recognised.
func load_conversations_conversationID_page(ctx context.Context) (any, error) {
...
func Loads() []*skgo.ServerLoad {
	return []*skgo.ServerLoad{
		skgo.NewServerLoad(skgo.LoadSpec{Module: "src/routes/conversations/+layout.server.ts", Run: load_conversations_layout}),
		skgo.NewServerLoad(skgo.LoadSpec{Module: "src/routes/conversations/[conversationID]/+page.server.ts", Run: load_conversations_conversationID_page}),
