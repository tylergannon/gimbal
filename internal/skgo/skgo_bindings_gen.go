...
// remote_watchRun answers src/routes/runs/[runID]/events.remote.ts#watchRun, a query.live.
//
// The argument is decoded by DecodeRoot7, generated by polytype for watchRun's
// own parameter type: a shape it does not admit is a 400 naming the path,
// never a zero value.
...
func remote_watchRun(ctx context.Context, call skgo.Call, yield func(any) error) error {
	if err := skgo.RequireArgument(call); err != nil {
		return err
	}
	...
	return skgo3.Skgo_watchRun(ctx, in, func(out skgo3.SkgoOut_watchRun) error {
...
// requestedArg_watchRun decodes one instance's argument for skgo.Requested, with the same
// decoder remote_watchRun uses.
func requestedArg_watchRun(arg any) (any, error) {
...
func Remotes() []*skgo.Remote {
	return []*skgo.Remote{
		...
		skgo.NewRemote(skgo.RemoteSpec{
			Kind:      skgo.KindLive,
			Module:    "src/routes/runs/[runID]/events.remote.ts",
			Name:      "watchRun",
			Fn:        skgo3.Skgo_watchRun,
			Live:      remote_watchRun,
			DecodeArg: requestedArg_watchRun,
		}),
		skgo.NewRemote(skgo.RemoteSpec{
			Kind:   skgo.KindForm,
...
func Transport() skgo.Transport {
	return skgo.Transport{
		"EventBatch": {
			Type:   reflect.TypeFor[skgotp0.EventBatch](),
			Encode: func(v any) (any, error) { return EncodeEventBatch(v.(skgotp0.EventBatch)) },
			Decode: func(raw any) (any, error) { return DecodeEventBatch(raw) },
		},
		"RunSnapshot": {
