...
var (
	__gen_jsonschema_compiled_Questions *jsonschema.Schema
)
...
func init() {
	...
	{
		var __zero Questions
		__gen_jsonschema_compiled_Questions = compile("Questions", __zero.Schema())
	}
}

func (Questions) Schema() json.RawMessage {
	const fileName = "jsonschema/Questions.json"
...
// ValidateJSON validates the given JSON bytes against the schema for Questions.
func (Questions) ValidateJSON(data []byte) error {
	inst, err := jsonschema.UnmarshalJSON(bytes.NewReader(data))
	if err != nil {
		return err
	}
	return __gen_jsonschema_compiled_Questions.Validate(inst)
}
