...
var (
	__gen_jsonschema_compiled_decision *jsonschema.Schema
)
...
func init() {
	...
	{
		var __zero decision
		__gen_jsonschema_compiled_decision = compile("decision", __zero.Schema())
	}
}

func (decision) Schema() json.RawMessage {
	const fileName = "jsonschema/decision.json"
...
// ValidateJSON validates the given JSON bytes against the schema for decision.
func (decision) ValidateJSON(data []byte) error {
	inst, err := jsonschema.UnmarshalJSON(bytes.NewReader(data))
	if err != nil {
		return err
	}
	return __gen_jsonschema_compiled_decision.Validate(inst)
}
