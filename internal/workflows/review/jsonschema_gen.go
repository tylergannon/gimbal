...
var (
	__gen_jsonschema_compiled_Result *jsonschema.Schema
)
...
func init() {
	...
	{
		var __zero Result
		__gen_jsonschema_compiled_Result = compile("Result", __zero.Schema())
	}
}

func (Result) Schema() json.RawMessage {
	const fileName = "jsonschema/Result.json"
...
// ValidateJSON validates the given JSON bytes against the schema for Result.
func (Result) ValidateJSON(data []byte) error {
	inst, err := jsonschema.UnmarshalJSON(bytes.NewReader(data))
	if err != nil {
		return err
	}
	return __gen_jsonschema_compiled_Result.Validate(inst)
}
