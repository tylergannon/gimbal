...
//
//	GIMBAL_LIVE=1 go test . -run TestLiveKillTurnByID -v
func TestLiveKillTurnByID(t *testing.T) {
	...
	defer cancel()
	instance, err := web.NewInstance(ctx, filepath.Join(logs, ".gimbal"), []string{logs}, web.WithNoWeb())
	if err != nil {
		t.Fatal(err)
	}
	...
	kill := gimbal.Killed{Target: turnID, By: "attest", Reason: "the operator killed this turn by id"}
	var killErr error
	var killWG sync.WaitGroup
	var first, second error
	dir := filepath.Join(logs, ".gimbal", "runs")
	start := time.Now()
	err = runtime.Run(ctx, "cancel-by-id", map[gimbal.WorkflowRole]gimbal.ModelBinding{"worker": {Adapter: codex.New(), Model: "gpt-5.6-luna"}}, func(ctx context.Context) error {
		return gimbal.Scope(ctx, "lap", func(ctx context.Context) error {
			session := gimbal.NewSession(ctx, "worker", workspace)
			killWG.Go(func() {
				time.Sleep(4 * time.Second)
				killErr = runtime.KillTurn(liveRunID(t, dir), turnID, kill.By, kill.Reason)
			})
			_, first = session.Generate[gimbal.Text](ctx, "Count from 1 to 400, one number per line, in your final answer. Do not use any tools and do not summarize; write out every number.")
			...
			text, err := session.Generate[gimbal.Text](ctx, "Answer in one sentence: what is a context in Go? Use no tools.")
			second = err
	...
	var killedRecords []gimbal.LifecycleRecord
	var ended []gimbal.TurnEnded
