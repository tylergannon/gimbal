...
// Loop is either a plain per-iteration loop or planner-directed dispatch.
// Planner is empty when the source ranges over Iterations, and its body runs
// under a fresh child scope named "iteration". For Tasks(goal, planner),
// Planner names the planner session and the body runs under a child scope
// named "task", where
// the runtime writes key "task". How many iterations or tasks there will be
